...
use super::{
    Context, Mode, PrettyPrinter, layout::flow::FlowItem, prelude::*,
    ...
    fn convert_markup_impl(
        ...
        let body = if scope == MarkupScope::InlineItem {
            ...
            match self.config.wrap_mode {
                WrapMode::SentencePerLine => PrettyPrinter::wrap_markup_sentence(self, ctx, &repr),
                WrapMode::Fill => PrettyPrinter::wrap_markup_fill(self, ctx, &repr),
...
#[derive(Default)]
pub(super) struct MarkupLine<'a> {
    pub(super) nodes: SmallVec<[&'a SyntaxNode; 4]>,
    pub(super) breaks: usize,
    mixed_text: bool,
}

pub(crate) struct MarkupRepr<'a> {
    pub(super) lines: Vec<MarkupLine<'a>>,
